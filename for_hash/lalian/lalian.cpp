...
node *lookup(char *str) {
    ...
    for (; np != NULL; np = np->next) {//这里是链地址法解决的冲突,返回的是第一个链表结点
...
char *search(char *name) {//对hash表查找特定元素(元素是字符串）
    node *np = lookup(name);
    if (np == NULL)
        return NULL;
    else
...
node *malloc_node(char *name, char *desc) {//在堆上为结点分配内存，并填充结点
...
*/
void displayHashTable() {//显示hash表元素（不包括空）
    node *np;
    unsigned int hashvalue;
